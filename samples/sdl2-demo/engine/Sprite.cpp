...
void Sprite::render(Renderer& rend)
{
	if (m_has_frames) {
		...
		Recti dst = Rectf((m_position - m_center), m_size ).as<int32_t>();
		rend.draw_texture( m_texture, src, dst, m_angle );
	} else {
		rend.draw_texture( m_texture, (m_position - m_center).as<int32_t>(), m_angle );
	}
}

Recti Sprite::frame_rect(int32_t frame)
{
	auto size = m_size.as<int32_t>();

	int32_t pos_x, pos_y, frame_y;
	pos_x = frame * size.x;
