...
void Renderer_SDL2::set_swap_interval(int32_t swap)
{
#ifndef EMSCRIPTEN
	int ret = SDL_GL_SetSwapInterval( (int)swap );
	if (swap == -1 && ret != 0) {
...
{
	auto* tex = dynamic_cast<Texture_SDL2*>(texture);
	if (tex) {
		SDL_Texture* s_tex = tex->m_texture;
		SDL_RenderCopyEx( m_rend, s_tex, (const SDL_Rect*) &src, (const SDL_Rect*) &dst, angle, nullptr,
						  SDL_FLIP_NONE );
	}
}
...
void Renderer_SDL2::draw_rect(const Recti& r, const Color& c)
{
	set_draw_color( c );
	SDL_RenderDrawRect( m_rend, (const SDL_Rect*) &r );
}
